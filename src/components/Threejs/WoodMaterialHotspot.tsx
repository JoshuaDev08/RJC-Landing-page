...
const WoodMaterialHotspot = ({ show, onMaterialChange, selectedMaterial }: Props) => {
  ...
  useFrame(() => {
    ...
    if (p <= 0.5) {
      ...
      positions.current[0] = START.x; positions.current[1] = START.y;
      positions.current[3] = tipX;    positions.current[4] = tipY;
      positions.current[6] = tipX;    positions.current[7] = tipY;
    } else {
      const t = (p - 0.5) / 0.5;
      ...
      // START → MID → tip
      positions.current[0] = START.x; positions.current[1] = START.y;
      positions.current[3] = MID.x;   positions.current[4] = MID.y;
      positions.current[6] = tipX;    positions.current[7] = tipY;
    }

    // Push updated positions to GPU
    if (lineGeoRef.current) {
      const attr = lineGeoRef.current.attributes.position as THREE.BufferAttribute;
  ...
  return (
    <>
      ...
      <line>
        ...
        <lineBasicMaterial color="#facc15" linewidth={1.5} />
      </line>
